...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.c_proj._is_attention_output = True  # Mark as attention output layer
...
class GPT2MLP(nn.Module):
    def __init__(self, intermediate_size, config):
        ...
        self.c_proj._is_mlp_output = True  # Mark as MLP output layer
...
class GPT2LMHeadModel(GPT2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        # If recurrent mode is enabled, call recurrent logic
        if getattr(self.config, "recurrent_model", False):
            # print("using recurrent model")
            # Get initial embedding
            if inputs_embeds is None:
                inputs_embeds0 = self.transformer.wte(input_ids)
            else:
            ...
            for iteration in range(self.config.pondering_steps):
                ...
                if getattr(self.config, "scale_embeds", False):
                    interpolated_embeds = torch.matmul(probs, self.transformer.wte.weight * embed_scale)
                else:
                    interpolated_embeds = torch.matmul(probs, self.transformer.wte.weight)
 
                current_embeds = current_embeds + interpolated_embeds

 
            final_outputs = self.transformer(
                input_ids=None,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
                position_ids=position_ids,
            ...
            if not return_dict:
                output = (lm_logits,) + final_outputs[1:]
