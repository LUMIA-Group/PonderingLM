...
class LlamaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.down_proj._is_mlp_output = True  # Mark as MLP output layer
    def forward(self, x):
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.o_proj._is_attention_output = True  # Mark as attention output layer
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            # Use truncated normal distribution to initialize weights, truncated at 3σ, variance is 2/(5*hidden_size)
            std = math.sqrt(2.0 / (5 * self.config.hidden_size))
            
            # For attention and MLP output projections, apply additional scaling based on network depth
            ...
            # Use truncated normal distribution to initialize embedding layer weights
            std = math.sqrt(2.0 / (5 * self.config.hidden_size))
            nn.init.trunc_normal_(module.weight.data, mean=0.0, std=std, a=-3*std, b=3*std)
            if module.padding_idx is not None:
                module.weight.data[module.padding_idx].zero_()
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        # If ponderinglm mode is enabled, call ponderinglm logic
        if getattr(self.config, "ponderinglm", False):
            ...
            # Pre-multiply initial embedding by pondering_steps
            ...
            # Iteratively optimize embedding
        ...
        # Original LlamaForCausalLM forward logic
        outputs = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
